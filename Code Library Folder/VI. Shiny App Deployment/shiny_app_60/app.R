...
ui <- fluidPage(
  titlePanel("60 Day Post Violation Predictions"),
  sidebarLayout(
    sidebarPanel(
...
server <- function(input, output, session) {
  # Load the Python pickle model
  python_exacutable <- here::here("venv/bin/python")
  use_python(python_exacutable, required = TRUE)
  joblib <- import("joblib")
  rf_model_60 <- joblib$load(here("rf_model_60.pkl"))
  ...
  observeEvent(input$predict_button, {
    ...
    python_result <- rf_model_60$predict(input_matrix)
    
    # Display prediction result
    output$prediction_output <- renderPrint({
      prediction_str <- paste("Python 60 Day Model Prediction:", python_result)
