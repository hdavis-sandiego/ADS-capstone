...
ui <- fluidPage(
  titlePanel("90 Day Post Violation Predictions"),
  sidebarLayout(
    sidebarPanel(
...
server <- function(input, output, session) {
  ...
  rf_model_90 <- joblib$load(here("rf_model_90.pkl"))
  ...
  observeEvent(input$predict_button, {
    ...
    python_result <- rf_model_90$predict(input_matrix)
    
    # Display prediction result
    output$prediction_output <- renderPrint({
      prediction_str <- paste("Python 90 Day Model Prediction:", python_result)
